...
const Product = ({item,buyButton,handleQty,onBuyBtnClick}) => {
  ...
  return (
    <div className='mx-auto px-8 border border-gray-200 rounded-lg'>
      ...
      {state.cart.some(p => p === item.id) ? <button className='bg-red-500 text-white p-2 mx-auto mb-4 block basis-2/4' onClick={()=>dispatch({type: "Delete", payload: item.id})} >Remove from cart</button> : <button className='bg-green-500 text-white p-2 mx-auto mb-4 block' onClick={()=>dispatch({type: "Add", payload: item.id})} >Add to cart</button>}
      </div>
    </div>
  )
}
